...
#     header_re = re.compile(rf"^({type_part})(?:\([A-Za-z0-9_\-:.]+\))?:\s+[^\n]{{1,72}}$")
...
#     # Body must be a list of bullet lines only. Each non-empty line must start with "- "
#     # and be at most 100 characters long (including the "- ").
...
#         if ln.strip() == "":
#             # allow single blank lines inside body only for readability
#             continue
#         if not ln.startswith("- "):
#             print('All non-empty body lines must start with "- "', file=sys.stderr)
#             print("Offending line:", ln, file=sys.stderr)
#             return False
#         seen_bullet = True
#     if not seen_bullet:
#         print('Commit body must contain at least one bulleted change line starting with "- "', file=sys.stderr)
#         return False
#     return True
...
#     if gitlint_rc != 0:
#         # gitlint may enforce rules that differ from this repository's custom
#         # validator (line lengths, etc.). Do not let gitlint failures block
#         # commits; print a note and continue with this script's checks which
#         # implement the authoritative repository rules.
#         print(
#             "Note: gitlint reported issues (rc=" + str(gitlint_rc) + ") — continuing with repository validator",
#             file=sys.stderr,
#         )
